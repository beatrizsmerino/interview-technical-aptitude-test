...
function getGlobalDependenciesInstalledNVM() {
	...
	console.groupCollapsed(
		"🚀 Global dependencies installed with the 'Manage Multiple Node Versions' NVM:",
	)
...
function getGlobalDependenciesInstalledVolta() {
	...
	let dependenceListInstalled = execSync(
		`volta list --current --format=plain`,
	).toString()
	dependenceListInstalled = dependenceListInstalled
		.split('\n')
		.filter(dependence => dependence.length > 0)
	dependenceListInstalled = dependenceListInstalled.map(
	...
	console.groupCollapsed(
		"🚀 Global dependencies installed with the 'Manage Multiple Node Versions' VOLTA:",
	)
	console.table(dependenceListInstalled)
	console.groupEnd()
	...
	if (usingVolta) {
		...
		Object.keys(toInstall).forEach(dependenceNameToInstall => {
			...
			if (!found) {
				...
				console.log(
					`➕ Installing dependency: ${dependenceNameToInstall}@${dependenceVersionToInstall}`,
				)
			} else {
				console.log(
					`✅ Dependency already installed: ${dependenceNameToInstall}@${dependenceVersionToInstall}`,
				)
			}
		})
		console.groupEnd()
